...
{
public:
    RulesAMST(const arma::mat& dataSet,
             ...
             const size_t referenceIndex)
    {
        ...
        if (queryComponentIndex != referenceComponentIndex)
        {
            ...
            double thevalue = ((f[queryIndex]+f[referenceIndex]))*distance;
    ...
    double Score(const size_t queryIndex,TreeType& referenceNode)
    {
        ...
        return neighborsDistances[queryComponentIndex] < (distance * (f[queryIndex] + referenceNode.Stat().minF()) / 2)
               ? DBL_MAX : distance;
    }
    ...
    //! In current implementation this oversimplified bound
