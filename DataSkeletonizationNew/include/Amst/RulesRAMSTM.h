#ifndef RULESRAMSTM_H
#define RULESRAMSTM_H
...
{
public:
    RulesRAMSTM(const arma::mat& dataSet,
                    ...
                    const size_t referenceIndex)
    {
        ...
        if (queryComponentIndex != referenceComponentIndex)
        {
            ...
            if (distance < epsilon)
            {
                double thevalue = f[referenceIndex]/2 + f[queryIndex]/2;
                // f[queryIndex]/2+

                if (thevalue < neighborsDistances[queryComponentIndex])
                {
    ...
    double Score(const size_t queryIndex,TreeType& referenceNode)
    {
        ...
        double minvalue = referenceNode.Stat().minF()/2 + f[queryIndex]/2;

        return neighborsDistances[queryComponentIndex] < minvalue
               ? DBL_MAX : minvalue ;
    }
    ...
    double Score(TreeType& queryNode, TreeType& referenceNode)
    {
        ...
        double fvalue = referenceNode.Stat().minF()/2 + queryNode.Stat().minF()/2 ;
