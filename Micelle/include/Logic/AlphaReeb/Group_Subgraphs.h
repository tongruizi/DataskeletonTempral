...
void Grouping ( MyGraphType const& g, vector<Data_Pt>const& cloud, int subgraph_iter, vector<Cluster>& cluster )
{
...
void Group_Subgraphs ( vector<MyGraphType>const& subgraph, vector<vector<Data_Pt>>const& subcloud, vector<Cluster>& cluster  )
{
	size_t num_subgraphs = subgraph.size();

	for (int subgraph_iter = 0; subgraph_iter < num_subgraphs; ++subgraph_iter)
	{
		if (boost::num_vertices( subgraph[subgraph_iter]) == 0 )
		{ continue; }
