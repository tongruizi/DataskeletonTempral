...
DualTreeTraverser_segment<RuleType>::DualTreeTraverser_segment(RuleType& rule) :
    rule(rule),
    numPrunes(0),
    numVisited(0),
    numScores(0),
...
DualTreeTraverser_segment<RuleType>::Traverse(
    BinarySpaceTree<MetricType, StatisticType, MatType, BoundType, SplitType>&
        queryNode,
    BinarySpaceTree<MetricType, StatisticType, MatType, BoundType, SplitType>&
        referenceNode)
{
  ...
  if (queryNode.IsLeaf() && referenceNode.IsLeaf())
  {
   ...
   // const size_t refEnd = referenceNode.Begin() + referenceNode.Count();
    for (int i=0; i<queryNode.NumPoints(); i++)
    {
      ...
      const double childScore = rule.Score(queryNode.Point(i), referenceNode);

      if (childScore == DBL_MAX)
        continue; // We can't improve this particular point.


            //!     rule.BaseCase(queryNode.Point(i), referenceNode.Point(t));
      for (int t=0; t<referenceNode.NumPoints();t++)
        rule.BaseCase(queryNode.Point(i), referenceNode.Point(t));
